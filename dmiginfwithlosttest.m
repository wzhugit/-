...
% btn2a=uicontrol('Units','normalized','Style','edit','Parent',p21,'Position',[0.55 0.9 0.3 0.05],'Callback',@fedit);
% btn2b=uicontrol('Units','normalized','Style','Pushbutton','Parent',p21,'Position',[0.6 0.6 0.25 0.15]);
% uicontrol(gcf,'Style','text','position',[0.01,0.9,0.1,0.05],'String','纸中污染物初始浓度');
%%
%反应损耗函数布局
...
uicontrol(p22,'Style','text','Units','normalized','FontUnits','normalized','FontSize',0.18,'position',[0.01,0.76,0.3,0.2],'BackgroundColor',[0.702 0.7804 1],'String','损耗模式选择');
uilostmodesel=uicontrol(p22,'Style','popupmenu','Units','normalized','position',[0.5,0.72,0.4,0.2],'String','无反应|at|<html>ba<sup>ct</sup></html>','Callback',@selfunc);
...
MF=PUFMigMFcall(cin,D,D,l1,l2,Ar,1,Th,M,nt);
MF(MF<=0)=0;
t=(0:1:nt)*Th/nt;

hold on

if get(uiradio1,'value')
    MFwithlost=MF.*(a*t+b*ab.^(c*t));
    MFwithlost(MFwithlost<=0)=0;
else
    MFwithlost=MF-(a*t+b*ab.^(c*t));
    MFwithlost(MFwithlost<=0)=0;
end
set(ax1,'visible','on');
plot(ax1,t,MFwithlost,'r','LineWidth',1.5);
hold on
...
function MFT1=PUFMigMFcall(cin,Dp,Dc,l1,l2,Ar,Kcp,Th,M,nt)
l=l1+l2;
VP=Ar*l1;
h=l/M;
m=floor(M*l1/l);
