function multimigfinitem
hold off
close all
format short
global uiCin uiD1 uiD2 uiD3 uil1 uil2 uil3 uik12 uik23 uiTh uiM uint uivf uihm uikpf ax1 uiA
figure('Name','多层有限食品迁移模型－质量','Units','normalized','Position',[0.1 0.1 0.85 0.75],'Color',[0.702 0.7804 1],'numbertitle','off');
axes('Units','normalized','Position',[0.25 0.9 0.4 0.1],'Visible','off');
text('Units','normalized','FontUnits','normalized','FontSize',0.65,'String','多层有限包装-有限食品迁移模型-质量');
...
uiTh=uicontrol(p21,'Units','normalized','Style','edit','Position',[0.6 0.45 0.25 0.04],'Tag','Thedit','String','500');
%%
...
for ij=m1+2:1:m1+m2+1
    MCT(1,1)=MCT(1,1)+0.5*h*Ar*(C(ij,1)+C(ij+1,1));
end
for ij=m1+m2+1+2:1:M+2
...
plot(ax1,(0:1:nt)*Th/nt,MFT1,'r','LineWidth',1.5);
hold on
